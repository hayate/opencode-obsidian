...
mutate(OS, "(project === null ? result.background.then(() => compare(null), () => compare(null)) : compare(project))", "undefined", AS, pattern="vault access")
mutate(OS, "(project === null ? result.background.then(() => compare(null), () => compare(null)) : compare(project))", "compare(project)", AS, pattern="told after the lines that say why")
mutate(OS, "      .catch((err: unknown) => this.tellOnce(entry, { level: \"error\", text: `vault file access: comparing", "      .catch((err: unknown) => void err && this.tellOnce(entry, { level: \"error\", text: `vault file access: comparing", AS, pattern="comparison that fails")
