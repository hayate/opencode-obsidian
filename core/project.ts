...
async function readOrigin(projectDir: string): Promise<string | null> {
  try {
    return (await readFile(join(projectDir, "remember", ORIGIN_FILE), "utf8")).trim() || null;
  } catch {
    return null;
  }
}
...
// Written once, on first use; never overwritten.
export async function recordOrigin(projectDir: string, origin: string): Promise<void> {
  ...
  try {
    await writeFile(join(projectDir, "remember", ORIGIN_FILE), `${origin}\n`, { flag: "wx" });
  } catch (err) {
